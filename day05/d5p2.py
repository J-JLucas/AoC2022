...
def stackMove(dishRack, moves, origin, dest):
    ...
    moves k objects from one stack to the other, slicing like 
    a deck of cards, (stack[n-moves] is the cut point)
    inputs: list dishRack, int moves, int origin, int dest
    returns: null
    '''
    temp = [] 
    # pop everything to be moved to temp stack object
    for i in range(moves):
        dish = dishRack[origin].pop()
        temp.append(dish)
    # push everything from temp stack onto final destination stack
    for i in range(moves):
        dish = temp.pop()
        dishRack[dest].append(dish)
    return
